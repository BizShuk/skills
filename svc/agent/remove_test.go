	...
	sel := RemoveSelection{
		Items: []InstalledItem{{
			...
			Locations: []InstalledLocation{
				{Agent: "claude-code", Scope: ScopeProject, Path: dir},
			},
		}},
	}

	skills, subagents, err := Remove(sel)
	require.NoError(t, err)
	assert.Equal(t, []string{"writer"}, skills)
...
func TestRemove_DeletesSubagentFile(t *testing.T) {
	...
	sel := RemoveSelection{
		Items: []InstalledItem{{
			...
			Locations: []InstalledLocation{
				{Agent: "claude-code", Scope: ScopeProject, Path: path},
			},
		}},
	}
	...
	require.NoError(t, err)
	assert.Nil(t, skills)
	assert.Equal(t, []string{"reviewer"}, subagents)
...
// several locations gets every copy removed and the name still surfaces
...
func TestRemove_MultipleLocationsAllDeleted(t *testing.T) {
	...
	sel := RemoveSelection{
		Items: []InstalledItem{{
			...
			Kind: InstalledSkill,
			Locations: []InstalledLocation{
				{Agent: "claude-code", Scope: ScopeProject, Path: dir1},
				{Agent: "antigravity", Scope: ScopeProject, Path: dir2},
			},
		}},
	}

	skills, _, err := Remove(sel)
	require.NoError(t, err)
	assert.Equal(t, []string{"writer"}, skills, "name should appear once")
...
func TestRemove_MissingPathIsNotAnError(t *testing.T) {
	sel := RemoveSelection{
		Items: []InstalledItem{{
			...
			Kind: InstalledSkill,
			Locations: []InstalledLocation{
				{Agent: "claude-code", Scope: ScopeProject, Path: "/nonexistent/path/to/ghost"},
			},
		}},
	}
	...
	assert.Equal(t, []string{"ghost"}, skills)
}
...
func TestRemove_PartialFailureReturnsErrAndDeletesWhatItCan(t *testing.T) {
	...
	sel := RemoveSelection{
		Items: []InstalledItem{{
			...
			Kind: InstalledSkill,
			Locations: []InstalledLocation{
				{Agent: "claude-code", Scope: ScopeProject, Path: good},
				{Agent: "antigravity", Scope: ScopeProject, Path: bad},
			},
		}},
	}
	...
	assert.Equal(t, []string{"writer"}, skills, "name still reported even on partial failure")
...
// skill and a subagent returns both name lists.
func TestRemove_DistinctKindsAccumulate(t *testing.T) {
	...
	sel := RemoveSelection{
		Items: []InstalledItem{
			{Name: "writer", Kind: InstalledSkill, Locations: []InstalledLocation{
				{Agent: "claude-code", Scope: ScopeProject, Path: skillDir},
			}},
			{Name: "reviewer", Kind: InstalledSubagent, Locations: []InstalledLocation{
				{Agent: "claude-code", Scope: ScopeProject, Path: subagentFile},
			}},
		},
	}
